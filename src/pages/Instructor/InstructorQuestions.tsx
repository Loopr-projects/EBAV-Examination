...
import { createQuestion, fetchQuestionsByCategory, deleteQuestion, copyQuestion, updateQuestion } from '../../api/questions'
...
export default function InstructorQuestions() {
  ...
  async function handleUpdateQuestion(guid: number, payload: Partial<CreateQuestionPayload>) {
    const updated = await updateQuestion(guid, payload)
    setQuestions((prev) => prev.map((q) => (q.guid === guid ? updated : q)))
    setSelectedQuestion(null)
  }
    ...
    <div className="dashboard-container">
          ...
          {selectedCategory && (
            <div className="info-card">
              ...
              {loadingQuestions ? (
                ...
                <QuestionList
                  questions={questions}
                  onEdit={(q) => { setSelectedQuestion(q); setShowEditQuestionModal(true) }}
      ...
      {showEditQuestionModal && selectedQuestion && (
        <EditQuestionModal
          ...
          onClose={() => { setShowEditQuestionModal(false); setSelectedQuestion(null) }}
